...
T_test = test_data_part[:,feature_dim * 6:feature_dim * 6 + 1]
Y_test = test_data_part[:,feature_dim * 6 + 1:feature_dim * 6 + 2]
...
T_train = train_data_part[:,feature_dim * 6:feature_dim * 6 + 1]
Y_train = train_data_part[:,feature_dim * 6 + 1:feature_dim * 6 + 2]
...
print 'Evaluate DPM Baseline1...6 windows...CutPoint=12months...MLP (all Dense) Model'
# Feature input part
...
merge_output = merge([dense1, dense2, dense3, dense4, dense5, dense6, dense_time], mode='sum')

activation = Activation('tanh')(merge_output)
prediction = Dense(1)(activation)

model = Model(input=[feature_input_win1, feature_input_win2, feature_input_win3, feature_input_win4, feature_input_win5, feature_input_win6, time_input], output=[prediction])

model.compile(optimizer='rmsprop', loss='mean_squared_error', metrics=['mean_squared_error'])

history = model.fit([X_train_win1, X_train_win2, X_train_win3, X_train_win4, X_train_win5, X_train_win6, T_train], [Y_train], nb_epoch=nb_epochs, batch_size=batch_sizes, validation_data=([X_test_win1, X_test_win2, X_test_win3, X_test_win4, X_test_win5, X_test_win6, T_test], [Y_test]))

print "Some history information\n"
print history.history
plot(model, to_file='0628_dpm_model_6win_mlp.png')
